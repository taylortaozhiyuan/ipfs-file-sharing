//main.js bug:server.json不更新到ipns
const fs = require('fs');
...
ipfsFile.add(buff).then((hash)=>{
	...
	//console.log(list_parse[dir_to_record][0]);
	if (!list_parse.hasOwnProperty(dir_to_record_name)){
	...
	else if (list_parse[dir_to_record][0] == hash1){
		console.log("文件没有改动");
		}
	else{				//文件有改动，开始更新链信息
		console.log("False");
		console.log("当前hash：",hash1);
		console.log("上次记录的hash：",list_parse[dir_to_record][0]);
		//let new_json = JSON.parse(fs.readFileSync('/home/taylor/ipfs_test/sample.json','utf-8'));
		...
		var myDate = new Date();
		name_res["last_revised_time"] = myDate.toLocaleString();
		console.log(JSON.parse(server_list)[dir_to_record][1]);
		object.links(JSON.parse(server_list)[dir_to_record][1]).then((content)=>{
		...
		new_json["Links"][0]["Name"] = JSON.stringify(name_res);
		new_json["Links"][0]["Hash"] = list_parse[dir_to_record][1];
		...
		//fs.writeFileSync(writePath,object_return);
				...
				let server_edit = JSON.parse(server_list);
				server_edit[dir_to_record][0] = hash1;
				server_edit[dir_to_record][1] = read.slice(9,55);
				//console.log(server_edit);
				fs.writeFileSync(server,JSON.stringify(server_edit));
